...
public:
	static Ref<File> open(const String& filePath, Mode mode);
	...
	sl_uint64 getSize();
	sl_bool seek(sl_int64 offset, Position from = positionBegin);
...
public:
	...
	{
		return (getAttributes(filePath) & attributeDirectory) != 0;
	}
	SLIB_INLINE static sl_bool isHidden(const String& filePath)
	{
		return (getAttributes(filePath) & attributeHidden) != 0;
	}
...
private:
	static sl_file _open(const String& filePath, Mode mode);
