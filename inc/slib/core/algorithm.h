...
{
public:
	SLIB_INLINE static int compare(const TYPE1& a, const TYPE2& b)
	{
		return (a < b) ? -1 : (a > b);
	}

	SLIB_INLINE static sl_bool equals(const TYPE1& a, const TYPE2& b)
	{
		return a == b;
	}
};
...
public:
	...
	SLIB_INLINE static sl_bool eqauls(const TYPE1& a, const TYPE2& b)
	{
		return a == b;
	}
};
...
{
public:
	SLIB_INLINE static sl_uint32 hash(const TYPE& v)
	{
		return sl_hash(v);
	}
};
...
public:
	...
	SLIB_INLINE static void sort(TYPE* list, sl_size size, sl_bool flagAsc = sl_true)
	{
		if (flagAsc) {
			sortAsc(list, size);
		} else {
...
public:
	...
	SLIB_INLINE static void sort(const TYPE* src, TYPE* dst, sl_size size, sl_bool flagAsc = sl_true)
	{
		if (flagAsc) {
