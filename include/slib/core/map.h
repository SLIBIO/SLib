...
namespace slib
{
...
#ifdef SLIB_SUPPORT_STD_TYPES
		CMap(const std::initializer_list< Pair<KT, VT> >& l) noexcept;
#endif
	...
	public:
		...
		void copyFrom_NoLock(const CMap& other) noexcept;

		void copyFrom(const CMap& other) noexcept;
	...
	public:
		...
		{
		public:
			EnumLockHelper(const Map<KT, VT, KEY_COMPARE>& map);
		public:
			NODE* node;
			Mutex* mutex;
			Ref<Referable> ref;
		...
		{
		public:
			EnumHelper(const Map<KT, VT, KEY_COMPARE>& map);
		public:
			NODE* node;
			Ref<Referable> ref;
		};
