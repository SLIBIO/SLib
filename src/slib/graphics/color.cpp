...
#define _COLOR_YUV420_I420(p, pU, pV, sY, sUV, S) \
	sY = ((W - 1) | 15) + 1; \
	sUV = (((sY >> 1) - 1) | 15) + 1; \
	pU = p + sY * H; \
	pV = pU + sUV * (H >> 1); \
...
#define _COLOR_YUV420_YV12(p, pU, pV, sY, sUV, S) \
	sY = ((W - 1) | 15) + 1; \
	sUV = (((sY >> 1) - 1) | 15) + 1; \
	pV = p + sY * H; \
	pU = pV + sUV * (H >> 1); \
...
#define _COLOR_YUV420_NV21(p, pU, pV, sY, sUV, S) \
	sY = W; \
	sUV = W; \
	pV = p + sY * H; \
	pU = pU + 1; \
...
#define _COLOR_YUV420_NV12(p, pU, pV, sY, sUV, S) \
	sY = W; \
	sUV = W; \
	pU = p + sY * H; \
	pV = pU + 1; \
...
#define _COLOR_YUV420_DEFINE_PARAMS(model, p, pU, pV, sY, sUV, S) \
	switch (model) { \
	case Color::YUV_I420: \
		_COLOR_YUV420_I420(p, pU, pV, sY, sUV, S); \
		break; \
	case Color::YUV_YV12: \
		_COLOR_YUV420_YV12(p, pU, pV, sY, sUV, S); \
		break; \
	case Color::YUV_NV21: \
		_COLOR_YUV420_NV21(p, pU, pV, sY, sUV, S); \
		break; \
	case Color::YUV_NV12: \
		_COLOR_YUV420_NV12(p, pU, pV, sY, sUV, S); \
		break; \
	default: \
		return; \
	}
...
static void _Color_convert_from_to_yuv420(sl_size width, sl_size height, sl_uint32 modelIn, const void* in, sl_uint32 modelOut, void* out)
{
	...
	if (modelIn == modelOut) {
		if (dataIn != dataOut) {
			sl_uint8* pUIn = 0; sl_uint8* pVIn = 0;
			sl_size pitchYIn = 0; sl_size pitchUVIn; sl_size strideUVIn = 0;
			_COLOR_YUV420_DEFINE_PARAMS(modelIn, dataIn, pUIn, pVIn, pitchYIn, pitchUVIn, strideUVIn);
			Base::copyMemory(dataOut, dataIn, pitchYIn * H + pitchUVIn * H);
		}
		return;
	}
	sl_uint8* pUIn = 0; sl_uint8* pVIn = 0;
	sl_size pitchYIn = 0; sl_size pitchUVIn; sl_size strideUVIn = 0;
	_COLOR_YUV420_DEFINE_PARAMS(modelIn, dataIn, pUIn, pVIn, pitchYIn, pitchUVIn, strideUVIn);
	sl_uint8* pUOut = 0; sl_uint8* pVOut = 0;
	sl_size pitchYOut = 0; sl_size pitchUVOut; sl_size strideUVOut = 0;
	_COLOR_YUV420_DEFINE_PARAMS(modelOut, dataOut, pUOut, pVOut, pitchYOut, pitchUVOut, strideUVOut);
...
#define _COLOR_SWITCH_DEFINE(modelOut) { \
	...
	for (i = 0; i < H2; i++) { \
		...
		sl_uint8* po = dataOut; \
		for (j = 0; j < W2; j++) { \
			c.setYUV(*piY, *piU, *piV); \
			c.write##modelOut(po); \
			c.setYUV(*(piY + 1), *piU, *piV); \
			c.write##modelOut(po + bytes##modelOut); \
			c.setYUV(*(piY + pitchYIn), *piU, *piV); \
			c.write##modelOut(po + pitchOut); \
			c.setYUV(*(piY + 1 + pitchYIn), *piU, *piV); \
...
static void _Color_convert_from_yuv420(sl_size width, sl_size height, sl_uint32 modelIn, const void* in, sl_uint32 modelOut, void* out, sl_reg pitchOut)
{
	...
	if (modelOut == Color::YUV444) {
		for (i = 0; i < H2; i++) {
			...
			sl_uint8* po = dataOut;
			for (j = 0; j < W2; j++) {
				_Color_set3(po, *piY, *piU, *piV);
				_Color_set3(po + 3, *(piY + 1), *piU, *piV);
				_Color_set3(po + pitchOut, *(piY + pitchYIn), *piU, *piV);
				_Color_set3(po + 3 + pitchOut, *(piY + 1 + pitchYIn), *piU, *piV);
				po += 6;
				piY += 2;
				piU += strideUVIn;
				piV += strideUVIn;
...
#define _COLOR_SWITCH_DEFINE(modelIn) { \
	...
	for (i = 0; i < H2; i++) { \
		...
		for (j = 0; j < W2; j++) { \
			...
			sl_uint8 _U, _V; \
			c.read##modelIn(pi); \
			c.getYUV(*poY, _U, _V); U += _U; V += _V; \
			c.read##modelIn(pi + bytes##modelIn); \
			c.getYUV(*(poY + 1), _U, _V); U += _U; V += _V; \
			c.read##modelIn(pi + pitchIn); \
			c.getYUV(*(poY + W), _U, _V); U += _U; V += _V; \
			c.read##modelIn(pi + bytes##modelIn + pitchIn); \
			c.getYUV(*(poY + 1 + W), _U, _V); U += _U; V += _V; \
			pi += bytes##modelIn + bytes##modelIn; \
			poY += 2; \
			*poU = (sl_uint8)(Math::clamp0_255(U >> 2)); \
			*poV = (sl_uint8)(Math::clamp0_255(V >> 2)); \
			poU += strideUVOut; \
			poV += strideUVOut; \
		} \
		dataIn += (pitchIn + pitchIn); \
		dataOut += (pitchYOut + pitchYOut); \
		pUOut += pitchUVOut; \
		pVOut += pitchUVOut; \
	} \
}
	...
	if (modelIn == Color::YUV444) {
		for (i = 0; i < H2; i++) {
			...
			for (j = 0; j < W2; j++) {
				sl_int32 U = 0, V = 0;
				sl_uint8 _U, _V;
				_Color_get3(pi, *poY, _U, _V); U += _U; V += _V;
				_Color_get3(pi + 3, *(poY + 1), _U, _V); U += _U; V += _V;
				_Color_get3(pi + pitchIn, *(poY + W), _U, _V); U += _U; V += _V;
				_Color_get3(pi + 3 + pitchIn, *(poY + 1 + W), _U, _V); U += _U; V += _V;
				pi += 6;
				poY += 2;
				*poU = (sl_uint8)(Math::clamp0_255(U >> 2));
				*poV = (sl_uint8)(Math::clamp0_255(V >> 2));
