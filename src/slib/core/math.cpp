...
namespace Math
{
	...
	float normalizeDegree(float v)
	{
		return _Math_normalizeDegree(v);
	}
	
	double normalizeDegree(double v)
	{
		return _Math_normalizeDegree(v);
	}
	
	float normalizeDegreeDistance(float v)
	{
		return normalizeDegree(v + 180) - 180;
	}
	
	double normalizeDegreeDistance(double v)
	{
		return normalizeDegree(v + 180) - 180;
	}
	...
	{
		T _cos = cos(angle);
		if (isAlmostZero(_cos) || isAlmostZero(sin(angle))) {
			return angle;
		}
		T PI2 = DualPI<T>();
		T stretched = arctan2(sin(angle) / abs(radiusY), cos(angle) / abs(radiusX));
		T revs_off = round(angle / PI2) - round(stretched / PI2);
		stretched += revs_off * PI2;
		return stretched;
	}
	
	float convertAngleFromEllipseToCircle(float angle, float radiusX, float radiusY)
	{
		return _Math_convertAngleFromEllipseToCircle(angle, radiusX, radiusY);
	}
	
	double convertAngleFromEllipseToCircle(double angle, double radiusX, double radiusY)
	{
		return _Math_convertAngleFromEllipseToCircle(angle, radiusX, radiusY);
	}
	...
	void randomMemory(void* _mem, sl_size size)
	{
