...
void STabView::setTabsCount(sl_uint32 nNew)
{
	...
	sl_uint32 nOrig = (sl_uint32)(items.getCount());
	if (nOrig == nNew) {
		return;
	}
	if (nOrig > nNew) {
...
void STabView::setTabLabel(sl_uint32 index, const String& _text)
{
	MutexLocker lock(getLocker());
	if (index < m_items.count()) {
		TabViewItem* item = m_items.getItemPtr(index);
...
void STabView::setTabContentView(sl_uint32 index, const Ref<View>& _view)
{
...
void STabView::onMouseEvent(UIEvent* ev)
{
	...
	if (ev->getAction() == actionMouseLeave) {
		m_indexHover = -1;
		invalidateTabBar();
...
void STabView::onDrawTab(Canvas* canvas, const Rectangle& rect, sl_uint32 index, const String& _label)
{
	...
	if (label.isNotEmpty()) {
		...
		if (m_flagVertical) {
			canvas->drawText(label, rc, font, labelColor, alignMiddleLeft);
		} else {
			canvas->drawText(label, rc, font, labelColor, alignMiddleCenter);
		}
	}
}
